    ...
    <section class="page">
      ...
      @if (view() === 'day') {
        ...
        <section class="calendar-week">
          @for (item of weekItems(); track item.id) { <article class="panel"><div class="panel-title"><h2>{{ item.date || 'Scheduled' }}</h2><span>{{ item.status }}</span></div><strong>{{ item.startTime || '-' }} - {{ item.endTime || '-' }}</strong><p class="muted">{{ item.type || 'roster' }}</p></article> } @empty { <section class="panel"><p class="empty">No upcoming calendar entries.</p></section> }
        </section>
      }
    </section>
  `,

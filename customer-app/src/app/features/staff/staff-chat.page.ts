...
@Component({ standalone: true, imports: [DatePipe, FormsModule, IonSpinner], template: `
  ...
  @if (os(); as data) { <section class="grid two chat-layout"><article class="panel"><div class="panel-title"><h2>Threads</h2><span>{{ threads().length }}</span></div><div class="list">@for (thread of threads(); track thread.id) { <button type="button" class="thread-row" [class.active-toggle]="thread.id === activeThreadId()" (click)="openThread(thread.id)"><strong>{{ thread.title }}</strong><small>{{ thread.messageCount || 0 }} messages · {{ thread.channel }}</small></button> } @empty { <p class="empty">No chat threads yet.</p> }</div><div class="panel-title mini"><h2>System feed</h2><span>{{ filteredNotifications().length }}</span></div><div class="list">@for (note of filteredNotifications().slice(0, 4); track note.id) { <div class="row"><div class="row-main"><strong>{{ note.title }}</strong><small>{{ note.createdAt ? (note.createdAt | date:'short') : '' }} · {{ note.body || note.status }}</small></div></div> }</div></article><article class="panel chat-panel"><div class="panel-title"><h2>Branch channel</h2><span>{{ staff.user()?.branchId || 'staff' }}</span></div><div class="chat-messages">@for (message of messages(); track message.id) { <article [class.mine]="message.senderStaffId === staff.user()?.staffId"><strong>{{ message.senderName || 'Staff' }}</strong><p>{{ message.body }}</p><small>{{ message.createdAt | date:'short' }}</small></article> } @empty { <p class="empty">No messages yet. Start the thread.</p> }</div><div class="chat-compose"><input [(ngModel)]="draft" placeholder="Message your team..." /><button class="link-button" type="button" (click)="send()">Send</button></div>@for (card of data.aiCoach.slice(0, 2); track card.title) { <p class="insight"><b>{{ card.title }}</b>: {{ card.action }}</p> }</article></section> }
...
export class StaffChatPage implements OnInit, OnDestroy {
  ...
  async load() { this.loading.set(true); try { const [os, threads] = await Promise.all([this.staff.enterpriseOs(), this.staff.chatThreads()]); this.os.set(os); this.threads.set(threads); if (threads[0]?.id) await this.openThread(threads[0].id); } finally { this.loading.set(false); } }
  async openThread(threadId: string) { this.activeThreadId.set(threadId); await this.refreshMessages(); }
  async refreshMessages() { if (!this.activeThreadId()) return; this.messages.set(await this.staff.chatMessages(this.activeThreadId())); }
  async send() { const body = this.draft.trim(); if (!body || !this.activeThreadId()) return; const message = await this.staff.sendChatMessage(this.activeThreadId(), body); this.messages.update((items) => items.some((item) => item.id === message.id) ? items : [...items, message]); this.draft = ""; }

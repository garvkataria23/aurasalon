  ...
  template: `
    <section class="page"><header class="page-head"><div><p class="eyebrow">Performance</p><h1>Performance intelligence</h1><p>Productivity, utilization, rating and improvement signals.</p></div></header>
      ...
      @if (os(); as data) {
        ...
        @if (canSeeRevenue()) { <section class="panel dark"><div class="panel-title"><h2>Revenue impact</h2><span>connected</span></div><h2>{{ data.performance.revenue | currency:'INR':'symbol':'1.0-0' }}</h2></section> }
      }
    </section>
  `,

      ...
      @if (canReadTasks() && today(); as data) {
        ...
        <section class="kanban-board">
          @for (column of columns; track column.status) {
            <article class="panel kanban-column">
              ...
              <div class="list">
                @for (task of tasksByStatus(column.status); track task.id) {
                  <div class="kanban-card"><strong>{{ task.title }}</strong><small>{{ task.priority || 'medium' }} · {{ task.dueAt ? (task.dueAt | date:'short') : 'no due date' }}</small><div class="row-actions"><span class="badge">{{ task.status }}</span>@if (canUpdateTasks() && task.status !== 'completed') { <button type="button" class="link-button" (click)="completeTask(task.id, task.version)">Done</button> }</div></div>

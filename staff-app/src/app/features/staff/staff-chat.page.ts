...
@Component({ standalone: true, imports: [DatePipe, FormsModule, IonSpinner], template: `
  <section class="page"><header class="page-head"><div><p class="eyebrow">Chat</p><h1>Team chat</h1><p>Dedicated communication workspace for staff updates.</p></div><div class="row-actions"><button class="link-button" [class.active-toggle]="filter() === 'all'" [attr.aria-pressed]="filter() === 'all'" type="button" (click)="filter.set('all')">All</button><button class="link-button" [class.active-toggle]="filter() === 'unread'" [attr.aria-pressed]="filter() === 'unread'" type="button" (click)="filter.set('unread')">Unread</button><button class="link-button" [class.active-toggle]="filter() === 'system'" [attr.aria-pressed]="filter() === 'system'" type="button" (click)="filter.set('system')">System</button></div></header>
...
export class StaffChatPage implements OnInit, OnDestroy {
  ...
  async load() { if (!this.canReadChat()) return; const generation = ++this.loadGeneration; this.loading.set(true); try { const [os, threads] = await Promise.all([this.staff.enterpriseOs(), this.staff.chatThreads()]); if (generation !== this.loadGeneration) return; this.os.set(os); this.threads.set(threads); if (threads[0]?.id && !this.activeThreadId()) await this.openThread(threads[0].id); } finally { if (generation === this.loadGeneration) this.loading.set(false); } }
  async openThread(threadId: string) { this.activeThreadId.set(threadId); await this.refreshMessages(); }
  async refreshMessages() { const threadId = this.activeThreadId(); if (!this.canReadChat() || !threadId) return; const generation = ++this.messageGeneration; const messages = await this.staff.chatMessages(threadId); if (generation === this.messageGeneration && threadId === this.activeThreadId()) this.messages.set(messages); }
  ...
  private async connectRealtime() {
    const url = await this.staff.realtimeSocketTicketUrl().catch(() => "");
    if (!url) return;

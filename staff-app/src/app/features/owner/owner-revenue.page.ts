...
@Component({
  ...
  template: `
    <article class="finance-page" [attr.aria-busy]="loading()">
      <header class="phase-header"><div><p>Financial performance</p><h1>Revenue</h1><span>Recorded sales, collections and liabilities—kept distinct.</span></div><button type="button" (click)="refresh()" [disabled]="loading()">{{ loading() ? 'Refreshing…' : 'Refresh' }}</button></header>
      ...
      @else if (data(); as view) {
        ...
        <section class="phase-panel drill-panel"><header><div><p>Source records</p><h2>{{ drillTitle() }}</h2></div><span>{{ drill()?.pagination?.total || 0 }} records</span></header>
          <div class="phase-filters"><label><span>Search</span><input type="search" [(ngModel)]="search" (keyup.enter)="loadDrilldown(1)" /></label><label><span>Status</span><input [(ngModel)]="status" (keyup.enter)="loadDrilldown(1)" /></label><label><span>Payment method</span><input [(ngModel)]="paymentMethodValue" [disabled]="!supportsPaymentMethod()" (keyup.enter)="loadDrilldown(1)" /></label><button type="button" (click)="loadDrilldown(1)" [disabled]="drillLoading()">Apply</button></div>
          @if (drillLoading() && !drill()) { <div class="table-loading" aria-label="Loading records"></div> }
          @else if (drillError()) { <p class="state-banner error" role="alert">{{ drillError() }} <button type="button" (click)="loadDrilldown(1)">Retry</button></p> }
...
export class OwnerRevenuePage {
  ...
  search = ""; status = ""; paymentMethodValue = ""; sortBy = "businessDate"; sortDirection: "asc" | "desc" = "desc"; private request = 0;
  constructor(readonly context: OwnerContextService, private readonly api: OwnerAppService, route: ActivatedRoute) {
    const requested = route.snapshot.queryParamMap.get("metric") as OwnerFinanceMetricKey | null; if (requested && METRICS.some((item) => item.key === requested)) this.selectedMetric.set(requested);
  ...
  async loadDrilldown(page: number): Promise<void> { this.drillLoading.set(true); this.drillError.set(""); try { const range = this.context.periodRange(); this.drill.set(await this.api.financeDrilldown(this.drillType(), { branchId: this.context.selectedBranchId() || "all", from: range.start, to: range.end, page, pageSize: 25, search: this.search, status: this.status, paymentMethod: this.paymentMethodValue, sortBy: this.sortBy, sortDirection: this.sortDirection })); } catch { this.drillError.set("These source records could not be loaded."); } finally { this.drillLoading.set(false); } }
  ...
  cell(value: OwnerReportCell, column: OwnerTypedColumn): string { if (value === null) return "Unavailable"; if (column.type === "money") return new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(Number(value) / 100); return String(value); }

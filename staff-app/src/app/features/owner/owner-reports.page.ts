...
export class OwnerReportsPage {
  ...
  formatTotal(key: string, value: number): string { return key.endsWith("Paise") ? new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(value / 100) : value.toLocaleString("en-IN"); }
  cell(value: OwnerReportCell, column: OwnerTypedColumn): string { if (value === null) return "Unavailable"; if (column.type === "money") return new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(Number(value) / 100); if (column.type === "date" && String(value).length >= 10) { const date = new Date(`${String(value).slice(0, 10)}T00:00:00Z`); return new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" }).format(date); } return String(value); }
  ...
  chartSummary(): string { const series = this.report()?.series || []; const total = series.reduce((sum, point) => sum + point.value, 0); const value = this.selectedKey() === "sales-summary" ? this.formatTotal("valuePaise", total) : total.toLocaleString("en-IN"); return `${series.length} recorded data points; combined value ${value}.`; }

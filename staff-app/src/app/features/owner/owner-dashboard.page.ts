...
export class OwnerDashboardPage implements OnDestroy {
  ...
  formatDate(value: string | null | undefined): string { if (!value) return "Date not supplied"; const date = new Date(value.length === 10 ? `${value}T00:00:00Z` : value); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short", year: "numeric", timeZone: value.length === 10 ? "UTC" : "Asia/Kolkata" }).format(date); }
  formatHour(value: string): string { const [hour] = value.split(":").map(Number); if (!Number.isFinite(hour)) return value; return new Intl.DateTimeFormat("en-IN", { hour: "numeric", timeZone: "UTC" }).format(new Date(Date.UTC(2020, 0, 1, hour))); }
  ...
  actionValue(item: OwnerActionItem): string { if (item.valuePaise !== undefined) return formatPaiseInr(item.valuePaise); if (item.value !== undefined && item.threshold !== undefined) return `${item.value.toLocaleString("en-IN")} · threshold ${item.threshold.toLocaleString("en-IN")}`; return item.value !== undefined ? item.value.toLocaleString("en-IN") : ""; }
  ...
  private value(value: number | null, unit: string): string { if (value === null) return "Unavailable"; return unit === "paise" ? formatPaiseInr(value) : value.toLocaleString("en-IN"); }
  private dateTime(value?: string): string { if (!value) return "Not refreshed"; const date = new Date(value); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short", hour: "numeric", minute: "2-digit", timeZone: "Asia/Kolkata" }).format(date); }

...
export class CommandCenterStore {
  ...
  load(): void {
    ...
    this.api.list<ApiRecord | ApiRecord[]>(config.primaryEndpoint)
      ...
      .subscribe({
        next: (response) => this.applyPrimary(response),
        error: (error) => this.error.set(error?.message || 'Unable to load command center data')
      });
    if (config.secondaryEndpoint) {
      this.api.list<ApiRecord | ApiRecord[]>(config.secondaryEndpoint).subscribe({
        next: (response) => this.applySecondary(response),
        error: () => undefined
      });
    }
  }
  ...
  runAction(): void {
    ...
    this.api.post<ApiRecord>(config.actionEndpoint, this.defaultPayload(config.module))
      ...
      .subscribe({
        next: () => this.load(),
        error: (error) => this.error.set(error?.message || 'Unable to run command center action')
      });
  }

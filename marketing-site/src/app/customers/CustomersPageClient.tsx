...
export default function CustomersPage() {
  ...
  return <>
    ...
    <section className="bg-white pb-20 md:pb-28"><Container><div className="mx-auto grid max-w-5xl border-l border-t border-aura-border sm:grid-cols-2">{evidence.map((item) => <article key={item} className="border-b border-r border-aura-border p-6 sm:p-8"><FileCheck2 className="h-5 w-5 text-aura-primary" /><h2 className="mt-5 font-display text-2xl text-aura-text">{t(`customerProof.${item}`)}</h2><p className="mt-2 text-sm leading-6 text-aura-text-secondary">{t(`customerProof.${item}.body`)}</p></article>)}</div></Container></section>

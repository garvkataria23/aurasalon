    ...
    return (
      <div className="absolute inset-0 z-0 pointer-events-none">
        <Canvas camera={{ position: [0, 0, 6], fov: 50 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }} style={{ background: "transparent" }}>
          ...
          <Orb position={[0, 0, 0]} color="#7C3AED" size={1} speed={1.5} distort={0.3} />
        </Canvas>
      </div>
    );
  }
  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none">
      <Canvas camera={{ position: [0, 0, 8], fov: 55 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }} style={{ background: "transparent" }}>
        ...
        <pointLight position={[-5, -5, -3]} intensity={0.4} color="#E879A8" />

        <Orb position={[-3, 1, -2]} color="#7C3AED" size={1.3} speed={1.5} distort={0.4} />
        <Orb position={[3, -1, -3]} color="#E879A8" size={0.8} speed={2} distort={0.3} />
        <Orb position={[0, 2.5, -4]} color="#F59E0B" size={0.6} speed={1.2} distort={0.2} />
      </Canvas>
    </div>
  );
}

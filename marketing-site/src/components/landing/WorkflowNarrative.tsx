    ...
    <section className="overflow-hidden bg-aura-bg-warm py-20 md:py-28">
      <Container size="wide">
        ...
        <ol className="relative mt-12 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {copy.workflow.steps.slice(0, compact ? 4 : 8).map((step, index) => <li key={step.title} className="relative min-w-0 rounded-2xl border border-aura-border bg-white p-5 shadow-[0_12px_35px_rgba(6,23,36,.05)]"><div className="flex items-center justify-between gap-3"><span className="text-[10px] font-bold uppercase tracking-[.14em] text-aura-primary">{step.tag}</span><span className="font-display text-lg text-aura-border-strong">{String(index + 1).padStart(2, "0")}</span></div><h3 className="mt-6 font-display text-2xl leading-tight text-aura-text">{step.title}</h3><p className="mt-3 text-sm leading-6 text-aura-text-secondary">{step.body}</p>{index < (compact ? 3 : 7) && <ArrowRight className="absolute -bottom-2 right-5 z-10 h-4 w-4 translate-y-full rotate-90 text-aura-primary-light md:right-6 xl:-right-2 xl:bottom-auto xl:top-1/2 xl:-translate-y-1/2 xl:rotate-0" aria-hidden="true" />}</li>)}
        </ol>

...
function ownerPayrollItem(row) {
  ...
  return {
    ...
    esicAmountPaise: paise(statutory.esic),
...
export class OwnerPeopleService {
  listStaff(access, query = {}) {
    ...
    const selected = db.prepare(`${sql} SELECT id, branchId, attendanceStatus FROM decorated WHERE ${where} ORDER BY lower(fullName), id LIMIT @limit OFFSET @offset`).all(params);
    const branchNames = new Map(branches.map((branch) => [branch.id, branch.name]));
    const items = selected.map((selectedRow) => ({ ...staffOsService.getStaff(selectedRow.id, scopedAccess(access, selectedRow.branchId)), branchName: branchNames.get(selectedRow.branchId) || "", attendanceStatus: selectedRow.attendanceStatus }));
